...
class prepare_data: 
    def __init__(self,rndseed,train_size,policy_type,demand_type,logging_type,feature_dist,dim,s_p,s_x):
        X_train=generate_feature(train_size,rndseed,feature_dist)
        P_train=historical_policy(X_train,logging_type,rndseed)
        P_train = np.maximum(P_train,3.0)#artificially lower  bound the price
        ...
        G,C = compute_Gram_matrix(P_train,X_train,P_test,dim,s_p,s_x)
        ...
        self.train_size,self.G,self.C = train_size,G,C; self.s_p=s_p; self.s_x = s_x
        self.rowSumHalfG,self.D3,self.arr_m,self.dim,self.demand_type = rowSumHalfG,D3,arr_m,dim,demand_type
...
class split_data: 
    def __init__(self,Train_data,whichHalf,rndseed):#rndseed determines how we split data
        ...
        self.rowSumHalfG,self.D3,self.arr_m,self.dim = rowSumHalfG,D3,arr_m,Train_data.dim
...
def compute_Gram_matrix_old(P_train, X_train, P_test, dim, sigma, returnDetails=0):
    train_size = len(P_train)
